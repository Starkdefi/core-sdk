...
describe('Currency', () => {
  ...
  describe('#equals', () => {
    it('ether on same chains is ether', () => {
      expect(Ether.onChain(SupportedChainId.MAINNET).equals(Ether.onChain(SupportedChainId.MAINNET)))
    })
    it('ether is not token0', () => {
      expect(Ether.onChain(SupportedChainId.MAINNET).equals(t0)).toStrictEqual(false)
    })
    ...
    it('token0 is equal to another token0', () => {
      expect(t0.equals(new Token(SupportedChainId.MAINNET, ADDRESS_ZERO, 18, 'symbol', 'name'))).toStrictEqual(true)
    })
  })
})